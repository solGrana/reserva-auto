document.addEventListener("DOMContentLoaded", async function () {
    ...
    /* function mostrarReservas(reservas) {
        ...
        function mostrarReservas(reservas) {
            listaReservas.innerHTML = ""; // Limpia la lista de reservas existente
            const hoy = new Date().setHours(0, 0, 0, 0); // Fecha de hoy sin hora
        
            reservas.forEach(res => {
                const fechaReserva = new Date(res.fecha).setHours(0, 0, 0, 0); // Fecha de la reserva sin hora
        
                if (fechaReserva < hoy) return; // Si la reserva ya pasó, la omitimos
